...
I18N = {
    ...
    # Прежняя подпись обещала «лица обезличены до записи на диск». Замер
    # 2026-09-06: из 48 кропов, записанных этой же сборкой, 4 несут резкие
    # лицевые кейпоинты НА ДИСКЕ. Обещание на странице про файлы, которых
    # читатель не видит, — это обещание, которое нечем подтвердить. Подпись
    # говорит про то, что читатель видит: про сами эти кропы.
    "cap.zone":  ("stratified by distance to the facade · every crop re-anonymised "
                  "by the detector before it is embedded here",
                  "стратифицировано по дистанции до фасада · каждый кроп заново "
                  "обезличен детектором перед вставкой на страницу",
                  "ファサードまでの距離で層化 · 各切り抜きは埋め込み前に検出器で再匿名化"),
    ...
    # НЕ «вошли»: метрики входа в проекте нет и никогда не считалось. Это
    # треки, подошедшие к фасаду ближе gaze_max_dist_m = 8 м. Ровно эту
    # подмену README уже назвал одной из пятнадцати ложных формулировок —
    # и она осталась стоять на самой странице.
    "fn.entered":  ("Came within 8 m of a storefront",
                    "Подошли к витрине ближе 8 м",
                    "店舗から8m以内に接近"),
    "fn.turned":   ("Turned toward a storefront", "Повернулись к витрине",
                    "店舗の方を向いた"),
    # Ступень воронки — СТРОГОЕ подмножество предыдущей: остановились И
    # повернулись И в зоне. Карточка KPI выше считает всех остановившихся, их
    # 7. Два числа про «остановились» в 400 пикселях друг от друга без разных
    # подписей читаются как противоречие — это ровно дефект «279 против 3»,
    # только на главной странице.
    "fn.stopped":  ("Stopped while turned toward it",
                    "Остановились, будучи повёрнутыми к ней",
                    "店舗を向いたまま立ち止まった"),
