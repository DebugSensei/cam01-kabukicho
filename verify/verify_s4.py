...
def main(argv=None) -> int:
    ...
    # прощал и то и другое, и [S4] возвращал 0 на пустом артефакте.
    if status is None:
        print(f"[S4] ПРОВАЛ: артефакт {ARTIFACT} отсутствует или без статуса")
        return 1
    if status == STATUS_SKELETON:
        print(f"[S4] ПРОВАЛ: артефакт {ARTIFACT} помечен "
