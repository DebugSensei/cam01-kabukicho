...
Требование гейта (CLAUDE.md): нет дыр в сегментах, fps и разрешение стабильны
...
REQUIREMENT = "нет дыр в сегментах, fps и разрешение стабильны"
...
def main(argv=None) -> int:
    ...
    # прощал и то и другое, и [S0] возвращал 0 на пустом артефакте.
    if status is None:
        print(f"[S0] ПРОВАЛ: артефакт {ARTIFACT} отсутствует или без статуса")
        return 1
    if status == STATUS_SKELETON:
        print(f"[S0] ПРОВАЛ: артефакт {ARTIFACT} помечен "
              f"status={STATUS_SKELETON} — данных нет, порог тут ни при чём")
        return 1

    print(f"[S0] НЕ РЕАЛИЗОВАН: метрика гейта не считается, результат не подтверждён")
    if args.allow_unmeasured:
        ...
        print(f"[S0] --allow-unmeasured: метрика не измерена, гейт пропущен")
        return 0
    return 1
