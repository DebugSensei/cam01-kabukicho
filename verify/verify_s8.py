...
def main(argv=None) -> int:
    ...
    # прощал и то и другое, и [S8] возвращал 0 на пустом артефакте.
    if status is None:
        print(f"[S8] ПРОВАЛ: артефакт {ARTIFACT} отсутствует или без статуса")
        return 1
    if status == STATUS_SKELETON:
        print(f"[S8] ПРОВАЛ: артефакт {ARTIFACT} помечен "
